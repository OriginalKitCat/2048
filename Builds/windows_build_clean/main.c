...
gchar *pop_sound_uri = NULL;
gchar *bonk_sound_uri = NULL;
gchar *background_uri = NULL;
...
static gchar *get_base_dir(void)
{
    wchar_t path[MAX_PATH];
    GetModuleFileNameW(NULL, path, MAX_PATH);
    gchar *utf8 = g_utf16_to_utf8(path, -1, NULL, NULL, NULL);
    gchar *dir = g_path_get_dirname(utf8);
    g_free(utf8);
    return dir;
}

static gchar *build_sound_uri(const gchar *filename)
{
    gchar *base = get_base_dir();
    if (!base) return NULL;
    gchar *filepath = g_build_filename(base, "share", "sounds", filename, NULL);
...
static gchar *build_icon_path(const gchar *filename)
{
    gchar *base = get_base_dir();
    if (!base) return NULL;
    gchar *path = g_build_filename(base, "share", "icons", "lucide-icons", filename, NULL);
    g_free(base);
    return path;
}
...
static gchar *build_sound_uri(const gchar *filename)
{
    ...
    if (!appdir) {
        g_warning("APPDIR not set. Not running inside AppImage?");
        return NULL;
    }
    gchar *filepath = g_build_filename(appdir,"usr","share","sounds",filename, NULL);
    gchar *uri = g_filename_to_uri(filepath, NULL, NULL);
    g_free(filepath);
    return uri;
}
...
static gchar *build_icon_path(const gchar *filename)
{
    ...
    return g_build_filename(appdir, "usr", "share", "icons", "lucide-icons", filename, NULL);
}
...
static gboolean on_sound_bus_message(GstBus *bus, GstMessage *msg, gpointer pipeline) {
...
void play_pop_sound() {
    ...
    if (!pipeline) {
        g_printerr("Failed to create GStreamer pipeline for pop sound\n");
        return;
    }
    g_object_set(pipeline, "uri", pop_sound_uri, NULL);
    GstBus *bus = gst_element_get_bus(pipeline);
    gst_bus_add_watch(bus, on_sound_bus_message, pipeline);
    gst_object_unref(bus);
    gst_element_set_state(pipeline, GST_STATE_PLAYING);
}
...
void play_bonk_sound() {
    GstElement *pipeline = gst_parse_launch("playbin uri=NULL", NULL);
    if (!pipeline) {
        g_printerr("Failed to create GStreamer pipeline for bonk sound\n");
        return;
    }
    g_object_set(pipeline, "uri", bonk_sound_uri, NULL);
    GstBus *bus = gst_element_get_bus(pipeline);
    gst_bus_add_watch(bus, on_sound_bus_message, pipeline);
    gst_object_unref(bus);
    gst_element_set_state(pipeline, GST_STATE_PLAYING);
...
static gboolean on_background_bus_message(GstBus *bus, GstMessage *msg, gpointer data) {
    if (GST_MESSAGE_TYPE(msg) == GST_MESSAGE_EOS) {
        gst_element_seek_simple(backgroundpipe, GST_FORMAT_TIME,
...
void init_background_music(const char *uri) {
    ...
    if (!backgroundpipe) {
        g_printerr("Failed to create background music pipeline\n");
        return;
    }
    g_object_set(backgroundpipe, "uri", uri, NULL);
    g_object_set(backgroundpipe, "volume", 0.3, NULL);

    GstBus *bus = gst_element_get_bus(backgroundpipe);
    gst_bus_add_watch(bus, on_background_bus_message, NULL);
    gst_object_unref(bus);
}
...
void stop_start_sound() {
    if (soundeffectes == TRUE) {
        soundeffectes = FALSE;
        gtk_image_set_from_file(GTK_IMAGE(soundeffectsicon), build_icon_path("volume-x.svg"));
        ...
        soundeffectes = TRUE;
        gtk_image_set_from_file(GTK_IMAGE(soundeffectsicon), build_icon_path("volume-2.svg"));
...
void activate(GtkApplication *app, gpointer data) {
    ...
    GtkWidget *menu_icon = gtk_image_new_from_file(build_icon_path("menu.svg"));
    ...
    GtkWidget *bgm_icon = gtk_image_new_from_file(build_icon_path("piano.svg"));
    ...
    soundeffectsicon = gtk_image_new_from_file(build_icon_path("volume-2.svg"));
    ...
    GtkWidget *restart_icon = gtk_image_new_from_file(build_icon_path("rotate-ccw.svg"));
...
int main(int argc, char **argv) {
    ...
    g_setenv("GST_PLUGIN_PATH", "C:\\msys64\\mingw64\\lib\\gstreamer-1.0", TRUE);
    g_setenv("GST_PLUGIN_SYSTEM_PATH", "C:\\msys64\\mingw64\\lib\\gstreamer-1.0", TRUE);
    g_setenv("GST_PLUGIN_SCANNER", "C:\\msys64\\mingw64\\bin\\gst-plugin-scanner.exe", TRUE);
    gst_init(&argc, &argv);

    pop_sound_uri = build_sound_uri("pop.mp3");
    bonk_sound_uri = build_sound_uri("bonk.mp3");
    background_uri = build_sound_uri("lofi-background.mp3");
    init_background_music(background_uri);

    g_print("Pop sound: %s\n", pop_sound_uri);
    g_print("Bonk sound: %s\n", bonk_sound_uri);
    g_print("Music: %s\n", background_uri);
