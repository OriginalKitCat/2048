...
static gchar *build_sound_uri(const gchar *filename)
{
    gchar *filepath = g_build_filename(base_dir, "share", "sounds", filename, NULL);
    gchar *uri = g_filename_to_uri(filepath, NULL, NULL);
    g_free(filepath);
    return uri;
}
...
static gchar *build_icon_path(const gchar *filename)
{
    return g_build_filename(base_dir, "share", "icons", "lucide-icons", filename, NULL);
}
...
void activate(GtkApplication *app, gpointer data) {
    ...
    gtk_grid_set_column_homogeneous(GTK_GRID(grid), TRUE);ff
...
int main(int argc, char **argv) {
    ...
    gchar *exe = realpath(argv[0], NULL);
    base_dir = g_path_get_dirname(exe);
    free(exe);
